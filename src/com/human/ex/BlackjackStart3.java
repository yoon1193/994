...
//		}
		...
		boolean palyer2Flag = true;
		Scanner sc = new Scanner(System.in);
		int score1Sum=0;
		int score2Sum=0;
		for (int j = 0; j < 10; j++) {
			if (player1Flag) {
				System.out.println("player1님 카드를 수령 하시겠습니까 0-no 1-yes");
			...
			if (palyer2Flag) {
				System.out.println("player2님 카드를 수령 하시겠습니까 0-no 1-yes");
				if (sc.nextLine().equals("0")) {
					palyer2Flag = false;
				}
				;
			}
			if (!(player1Flag || palyer2Flag)) {
				break;
			}
			if (player1Flag) {
			...
			//A가 1 or 11 ?
			for (int i = 0; i < player2Index; i++) {
				if (player2Deck[i] % 13 == 0) {// 카드중 A가 있을떄
					if ((score2Sum + 10) <= 21) {// A카드+2장카드+10 이 21보다 작거나 같으면 10을 더해줘 A를11로 판별한다
						score2Sum += 10;
					}
				}
			}
